...
def get_stock_data():
    ...
    for root, dirs, files in os.walk('/public/stock/tackder/history/day/data'):
        if files:
            for f in files:
                if '.csv' in f:
    ...
    for code in stock_code_list:
        # 此处为股票数据文件的本地路径，请自行修改
        stock_data = pd.read_csv('/public/stock/tackder/history/day/data/' + code + '.csv', parse_dates=['date'])
